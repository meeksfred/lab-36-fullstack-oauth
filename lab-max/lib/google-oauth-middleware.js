...
module.exports = function(req, res, next){
  ...
  .then( response => {
    ...
    tokenTTL = response.body.expires_in; // how long the access token will work in seconds, def 3600
    return request.get('https://www.googleapis.com/plus/v1/people/me/openIdConnect')
    .set('Authorization', `Bearer ${response.body.access_token}`) // gives token, refresh token, timetoLive
  })
  .then( response => {
    req.googleOAUTH = {
